...
CCT_Config Read_CCT_Config(RedisModuleCtx *ctx, std::string config_file_path_str) {
    ...
    if(config_read == false) {
        ...
        if( !config[DEFAULT_CONFIG_SECTION][CCT_SEND_OLD_VALUE_CONFIG].empty() ) {
            try{
                cct_config.CCT_SEND_OLD_VALUE_CFG = std::stoi(config[DEFAULT_CONFIG_SECTION][CCT_SEND_OLD_VALUE_CONFIG]);
            } catch (std::invalid_argument const& ex) {
                LOG(ctx, REDISMODULE_LOGLEVEL_WARNING , "Module config value " + CCT_SEND_OLD_VALUE_CONFIG  + " has failed to read value.");
            }
