...
void Add_Tracking_Query(RedisModuleCtx *ctx, RedisModuleString *query, std::string client_name, const std::vector<std::string> &key_ids) {
    ...
    // Save the Query:{Keys}
    std::string query_client_key_name_str = CCT_MODULE_QUERY_2_KEY + query_term + CCT_MODULE_KEY_SEPERATOR + query_attribute + CCT_MODULE_KEY_SEPERATOR + client_name;
    ...
    std::string query_name_as_value_str = query_term + CCT_MODULE_KEY_SEPERATOR + query_attribute + CCT_MODULE_KEY_SEPERATOR + client_name;
    for (const auto& it : key_ids) {
        std::string key_key_name_str = CCT_MODULE_KEY_2_QUERY + CCT_MODULE_KEY_SEPERATOR + it;  
