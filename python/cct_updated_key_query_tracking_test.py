...
def test_updated_key_match_new_query_while_not_mathing_old_matching_query():
    ...
    client1.execute_command(
        "CCT.FT.SEARCH " + cct_prepare.TEST_INDEX_NAME + " @User\\.ID:{" + query_value + "}")  # match first

    query_value = "bbb"
    client1.execute_command(
        "CCT.FT.SEARCH " + cct_prepare.TEST_INDEX_NAME + " @User\\.PASSPORT:{" + query_value + "}")  # match second
...
def test_updated_key_match_same_queries_one_client_multi_result_with_base():
    ...
    producer.ft(TEST_INDEX_NAME).create_index(schema, definition=IndexDefinition(prefix=[TEST_INDEX_PREFIX],
    ...
    query_value = "a_data"
    client1.execute_command(
        "CCT.FT.SEARCH " + cct_prepare.TEST_INDEX_NAME + " @user\\.a:{" + query_value + "}" + " LIMIT 0 100")  # match first
...
def test_updated_key_match_same_queries_one_client_multi_result():
    ...
    query_value = "a_data"
    client1.execute_command(
        "CCT.FT.SEARCH " + cct_prepare.TEST_INDEX_NAME + " @a:{" + query_value + "}" + " LIMIT 0 100")  # match first
...
def test_updated_key_match_same_queries_one_client_mixed_data():
    ...
    res = client1.execute_command(
        "CCT.FT.SEARCH " + cct_prepare.TEST_INDEX_NAME + " @d\\.d_1:{" + query_value + "}" + " LIMIT 0 100")  # match first
    print(res)
